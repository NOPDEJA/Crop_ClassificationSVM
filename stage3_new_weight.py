...
UPDATED: Implements explicit "Rebalance by LU" logic similar to Stage-1's 
superclass rebalancing (Pre/Post count logging, explicit capping, shuffling) 
...
Notes:
 ...
 - Uses rebalance_lu_distribution to cap dominant LU codes before splitting.
...
# Config
# -----------------------
NPZ = "./2018/aligned_features/svm_add_data_features_labels.npz"
STAGE1_PRED = "./2018/stage1_svm_weight_scale_increased.npy"
STAGE1_MODEL = "./2018/stage1_svm_weight_scale_increased.joblib"
STAGE2_PRED = "./2018/stage2_weighted_increased.npy"
STAGE2_MODEL = "./2018/stage2_weighted_increased_model.joblib"
...
OUT_MODEL_TPL = "stage3_{grp}_weightscale_model2.joblib"
OUT_REPORT_TPL = "stage3_{grp}_weightscale_report2.csv"
OUT_TEST_PROB_TPL = "stage3_{grp}_weightscale_test_prob2.npy"
OUT_TEST_PRED_TPL = "stage3_{grp}_weightscale_test_pred2.npy"
OUT_META_TPL = "stage3_{grp}_weightscale_meta2.json"
OUT_CONF_TPL = "stage3_{grp}_weightscale_confusion_matrix2.csv"
OUT_TOP_META = "stage3_weightscale_meta2.json"
...
def rebalance_lu_distribution(X, y, cap=PER_LU_CAP):
    """
    Mimics Stage-1 'rebalance_by_superclass' logic:
    1. Check Pre-counts.
    2. Cap indices per LU code.
    3. Shuffle and return.
    """
    print(f"--- Rebalancing LU distribution (Cap={cap}) ---")
    rng = np.random.default_rng(RANDOM_STATE)
    partsX, partsy = [], []
    
    # Log Pre-rebalance counts
    pre_counts = Counter(y)
    print(f"  Pre-rebalance counts: {dict(sorted(pre_counts.items()))}")

    for c in np.unique(y):
        idxs = np.flatnonzero(y == c)
        keep_n = min(len(idxs), cap)
        
        # Sample if needed, otherwise take all
        if len(idxs) > keep_n:
    ...
    # Shuffle (crucial, same as Stage-1)
    perm = rng.permutation(Xc.shape[0])
    Xc, yc = Xc[perm], yc[perm]
    
    # Log Post-rebalance counts
    post_counts = Counter(yc)
    print(f"  Post-rebalance counts: {dict(sorted(post_counts.items()))}")
    return Xc, yc
...
def sample_per_lu_train_only(X_train, y_train, target_per_lu=TARGET_PER_LU,
    ...
    """
    Secondary sampling specifically for TRAINING set to boost minorities
    before fitting.
    """
    rng = np.random.default_rng(RANDOM_STATE)
    lus = np.unique(y_train)
    partsX, partsy = [], []
    print("  --- Train-set optimization (Upsampling/Balancing) ---")
    for c in lus:
        ...
        # print(f"    LU {int(c)}: {n} -> {len(chosen)} (tgt={effective_target})")
...
if __name__ == "__main__":
    print("=== Stage-3 with weight-scaling + Stage-1 style Rebalancing : start ===")
    ...
    for subclass_label, lu_list in SUBCLASS_TO_CODES.items():
        ...
        if subclass_label == SUBCLASS_LABELS.get("other_econ"):
            print("WARNING: 'other_econ' subclass may be heavily filtered by Stage-2 predictions.")
        ...
        # 2. Rebalance (Stage-1 Style: Cap & Shuffle)
        Xc, yc = rebalance_lu_distribution(X_keep, y_keep, cap=PER_LU_CAP)
        ...
        # 4. Train-set Optimization (Upsampling if needed)
        print("  Step 4: manual per-LU upsample to reach TARGET_PER_LU (if configured)")
        X_tr_pre, y_tr_pre = sample_per_lu_train_only(X_tr, y_tr, target_per_lu=TARGET_PER_LU,
                                                     upsample=UPSAMPLE_SMALL, conditional=CONDITIONAL_UPSAMPLE,
                                                     min_trigger=MIN_UPSAMPLE_TRIGGER)
        
        print("  Step 5: ensure minimum total samples.")
        ...
        # Apply per-feature weight scale
    ...
    print("Stage-3 with weight-scaling + Stage-1 rebalancing complete.")